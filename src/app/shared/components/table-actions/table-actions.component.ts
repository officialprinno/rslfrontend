import { ChangeDetectionStrategy, Component, input, output } from '@angular/core';
import { RouterLink } from '@angular/router';

export interface TableAction {
  id: string;
  label: string;
  icon: 'view' | 'edit' | 'delete';
  danger?: boolean;
  hidden?: boolean;
  routerLink?: string | unknown[];
}

@Component({
  selector: 'app-table-actions',
  imports: [RouterLink],
  template: `
    <div class="table-actions" role="group" [attr.aria-label]="ariaLabel()">
      @for (action of visibleActions(); track action.id) {
        @if (action.routerLink) {
          <a
            [routerLink]="action.routerLink"
            class="table-action-btn"
            [class.table-action-btn--danger]="action.danger"
            [attr.title]="action.label"
            [attr.aria-label]="action.label"
          >
            @if (action.icon === 'view') {
              <svg class="table-action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
            } @else if (action.icon === 'edit') {
              <svg class="table-action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            } @else {
              <svg class="table-action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
            }
          </a>
        } @else {
          <button
            type="button"
            class="table-action-btn"
            [class.table-action-btn--danger]="action.danger"
            [attr.title]="action.label"
            [attr.aria-label]="action.label"
            (click)="actionClick.emit(action.id)"
          >
            @if (action.icon === 'view') {
              <svg class="table-action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
            } @else if (action.icon === 'edit') {
              <svg class="table-action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            } @else {
              <svg class="table-action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
            }
          </button>
        }
      }
    </div>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TableActionsComponent {
  readonly actions = input<TableAction[]>([]);
  readonly ariaLabel = input('Row actions');

  readonly actionClick = output<string>();

  visibleActions(): TableAction[] {
    return this.actions().filter((a) => !a.hidden);
  }
}
